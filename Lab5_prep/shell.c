...
extern unsigned long NumDataLost;
...
//      UART_OutString(_SH_getVar(command.arguments[0]));
    ...
    case 'q':
      ...
      break;
    case 'm':
      // change scope mode between voltage versus time/frequency
      exitCode = ADC_ChangeScopeMode();
      break;
    case 'f':
      // enable/disable FIR filter
      exitCode = ADC_ToggleFilter();
      break;
    case 'd':
      // dump data lost
      UART_OutString("DataLost = "); UART_OutUDec(NumDataLost);
      exitCode = 0;
      break;
    case 'r':
      // reset data lost
      NumDataLost = 0;
      UART_OutString("Cleared DataLost");
      exitCode = 0;
      break;
    case 'z':
      // print voltage log
      exitCode = SH_PrintVoltageLog();
      break;
    case 'x':
      // clear voltage log
      exitCode = SH_ClearVolatageLog();
      UART_OutString("Cleared Voltage Log");
      break;
    // TODO - add commands to manage the file system (format, directory, print file, delete file)
    case '0':
      exitCode = eFile_Format();
      UART_OutString("");
      break;
    case '1':
      exitCode = eFile_Directory(&UART_OutChar);
      break;
    case '2':
      exitCode = eFile_PrintFile(&UART_OutChar, "f_name");
      break;
    case '3':
      exitCode = eFile_Delete("f_name");
      break;
    default:
...
extern unsigned long VoltageLog[BUFF_LEN];
extern int VLindex;
int SH_PrintVoltageLog(void) {
  int i;
  // output as tab-seperated values
  for(i = 0; i < VLindex; i++) {
    UART_OutUDec(VoltageLog[i]);UART_OutString("\t");
  }
  return 0;
}
