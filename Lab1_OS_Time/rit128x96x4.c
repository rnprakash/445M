...
/* 16-bit OLED grayscale value */
static unsigned char _OLED_color = 15;

/* Set the color of the OLED output
 * param: unsigned char color, 16-bit grayscale color
 */
 ...
 * return: unsigned char _OLED_color, 16-bit grayscale color
 */
 ...
 * This function outputs a given string onto the top OLED screen
...
static void _OLED_Message(int device, unsigned int line, const char * string, unsigned char value)
{
		ASSERT(line < 5 && device < 2 && device >= 0);
		RIT128x96x4StringDraw(string, 0, device * 48 + (line * 8) + OLED_BUFFER, value);
}

/*static int _OLED_String_Count(const char* string, char c)
{
	int count = 0;
	while(*string)
		if(*(string++) == c)
