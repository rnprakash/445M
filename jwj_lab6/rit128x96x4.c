...
void OLED_TxtMessage (int device, int line, char*string){
		long row = ((device * 48) + (line * 12)); //device is either 0 or 1. If 0, start at top. If 1, start at row 48 (half way down)
		OS_bWait(&OLEDFree);
		RIT128x96x4StringDraw("                       ", 0, row, 8); //clear line
	  RIT128x96x4StringDraw(string, 0, row, 8);
	  OS_bSignal(&OLEDFree);
}
...
//*****************************************************************************
//
//writes a message to one of the two logical displays
//device: 0 or 1 : which logical display to write to
//line: 0 to 3: which line on the display to write to
//string: pointer to string to write
//value: number to display
//
//*****************************************************************************
void OLED_Message (int device, int line, char *string, long value){ char sValue[30]; //holds string value for value parameter
	//display 1 starts half way down, or 48 pixels.
	//each line is 8 pixels high, separate line by 12 pixels for spacing
	long row = ((device * 48) + (line * 12)); //device is either 0 or 1. If 0, start at top. If 1, start at row 48 (half way down)
	OS_bWait(&OLEDFree);
	RIT128x96x4StringDraw("                       ", 0, row, 8); //clear line
	sprintf(sValue, "%s %d", string, value); //concatenate string and long parameters into 1 string for output
	RIT128x96x4StringDraw(sValue, 0, row, 8);
	OS_bSignal(&OLEDFree);
