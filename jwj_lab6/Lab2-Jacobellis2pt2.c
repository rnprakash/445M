...
unsigned long myId = OS_Id(); 
  ...
  OLED_Message(1,3,"0.1u Jitter=",dasTcb->MaxJitter-dasTcb->MinJitter);
  OS_Kill();  // done
} 
...
// //  OS_AddPeriodicThread(&TaskA,PERIODIC_TIME_1MS,0);           // 1 ms, higher priority
// //  OS_AddPeriodicThread(&TaskB,2*PERIODIC_TIME_1MS,1);         // 2 ms, lower priority
//   OS_AddPeriodicThread(&TaskA,1498,0);           // 1 ms, higher priority
//   OS_AddPeriodicThread(&TaskB,1000,1);         // 2 ms, lower priority
...
int main(void){
	...
	OS_AddThread(&PID, 128, 0);
	OS_AddThread(&Interpreter, 128, 0);
	dasTcb = OS_AddPeriodicThread(&DAS, 1000, 0);
	
	OS_AddButtonTask(&ButtonPush, 1);
	OS_AddDownTask(&ButtonPush, 2);
