...
void SysTick_Handler(void){long delay;
  ...
  do{
    ...
    if((tempPt->priority < max) && (tempPt->sleepState == 0)){
      max = tempPt->priority;
      NextRunPt = tempPt;
    }   
    tempPt= tempPt->next;
...
void OS_EnableInterrupts(void){long endTime, startTime, timeDifference, i;
	i = GetInterruptStatus();
	startTime = StartCriticalTime;
	EnableInterrupts();
	PD4 = 0x0;
	if((i&0x01)){ //if interrupts were actually disabled
	PD2 = 0x0;
	endTime = OS_Time();
	timeDifference = OS_TimeDifference(endTime, startTime)/10;
...
void OS_EndCritical(long i){long endTime, startTime, timeDifference;

	startTime = StartCriticalTime;
	EndCritical(i);
	PD5 = 0x0;
	if(!(i&0x1)){ //if interrupts are actually getting reenabled (if I was 0, interrupts are getting enabled)
		PD3 = 0x0;
		endTime = OS_Time();
		timeDifference = OS_TimeDifference(endTime, startTime)/10;
		if(MaxCritical<timeDifference){
