...
//  OS_MeasureJitter();         // print jitter information
//   printf("\n\r\n\r");
  OS_Kill();
}
...
int Testmain5(void){       // Testmain5
  ...
  OS_Init();           // initialize, disable interrupts
  NumCreated = 0 ;
  //NumCreated += OS_AddThread(&Thread6,TaskAPeriod,2); 
  //NumCreated += OS_AddThread(&Thread7,TaskBPeriod,1); 
  TaskAID = OS_Add_Periodic_Thread(&TaskA,TIME_1MS,0);           // 1 ms, higher priority
  TaskBID = OS_Add_Periodic_Thread(&TaskB,2*TIME_1MS,1);         // 2 ms, lower priority
