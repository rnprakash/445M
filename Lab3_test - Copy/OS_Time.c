...
long MaxJitter[OS_MAX_TASKS] = {0};    // largest time difference between interrupt trigger and running thread
long MinJitter[OS_MAX_TASKS] = {10000000};    // smallest time difference between interrupt trigger and running thread
unsigned long const JitterSize = JITTERSIZE;
unsigned long JitterHistogram[OS_MAX_TASKS][JITTERSIZE];
...
void MeasureJitter(int taskID, unsigned long period) {
  static unsigned long LastTime[OS_MAX_TASKS];
  static char firstTime[OS_MAX_TASKS] = {1};
  int index;
  ...
  if(firstTime[taskID]) {
    ...
    jitter = OS_TimeDifference(period, OS_TimeDifference(LastTime[taskID],thisTime))/50;  // in usec
