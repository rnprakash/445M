...
unsigned long myId = OS_Id(); 
  OLED_Message(BOTTOM, "NumCreated =", NumCreated); 
  if(NumSamples < RUNLENGTH){   // finite time run
    for(i=0;i<20;i++){  // runs for 2 seconds
      OS_Sleep(50);     // set this to sleep for 0.1 sec
    }
  }
  OLED_Message(BOTTOM,"PIDWork    =",PIDWork);
  OLED_Message(BOTTOM,"DataLost   =",DataLost);
  OLED_Message(BOTTOM,"0.1u Jitter=",MaxJitter-MinJitter);
  OS_Kill();  // done
} 
...
unsigned long data,voltage;
  OLED_Message(TOP,"Run length is",(RUNLENGTH)/1000);   // top half used for Display
  while(NumSamples < RUNLENGTH) { 
    OLED_Message(TOP,"Time left is",(RUNLENGTH-NumSamples)/1000);   // top half used for Display
    data = OS_MailBox_Recv();
    voltage = 3000*data/1024;               // calibrate your device so voltage is in mV
    OLED_Message(TOP,"v(mV) =",voltage);  
  } 
  OS_Kill();  // done
} 
...
//*******attach background tasks***********
  ...
  OS_Add_Periodic_Thread(&DAS,PERIOD,1); // 2 kHz real time sampling
...
int Testmain5(void){       // Testmain5
  ...
  OS_Add_Periodic_Thread(&TaskA,TIME_1MS,0);           // 1 ms, higher priority
  OS_Add_Periodic_Thread(&TaskB,2*TIME_1MS,1);         // 2 ms, lower priority
...
void OutputThread(void){  // foreground thread
  OLED_Out(TOP, "EE345M/EE380L, Lab 3 Preparation 4");
  while(SignalCount1+SignalCount2+SignalCount3<100*MAXCOUNT){
    OS_Sleep(1000);   // 1 second
    OLED_Out(TOP, ".");
  }       
  OLED_Out(TOP, "Done");
  OLED_Message(TOP, "Signalled =", SignalCount1+SignalCount2+SignalCount3);
	OLED_Message(TOP, "Waited =",WaitCount1+WaitCount2+WaitCount3);
...
int main(void){      // Testmain6
  ...
  OS_Add_Periodic_Thread(&Signal1,(799*TIME_1MS)/1000,0);   // 0.799 ms, higher priority
  OS_Add_Periodic_Thread(&Signal2,(1111*TIME_1MS)/1000,1);  // 1.111 ms, lower priority
  NumCreated = 0 ;
