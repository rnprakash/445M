...
int OS_AddThread(void(*task)(void), unsigned long stackSize, unsigned long priority) {
  ...
  if(_RunPt == NULL) {
    ...
    if(i == _OS_MAX_THREADS) {
      ...
      thread = &_threads[i];
      // add thread in order
      _OS_InitThread(thread, task, temp->next, /*temp->prev*/ NULL, tid, 0, 0, priority)
...
void OS_FindNextThread(void)
{
  _TCB *temp, *head;
	...
	temp = _RunPt;
	while(temp->next != NULL && (temp->next->priority <= _RunPt->priority || temp->next->block || temp->next->sleep))
		temp = temp->next;
