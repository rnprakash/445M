...
#define PINS 0x07 // port b pins used to profile
...
//   oLED_Message(1,0,"NumCreated =",NumCreated); 
  ...
  OLED_Out(BOTTOM, str);
   sprintf(str, "0.1u Jitter= %d", OS_getMaxJitter(DAS_id) - OS_getMinJitter(DAS_id));
   OLED_Out(BOTTOM, str);
...
void Display(void){ 
  ...
  NumCreated += OS_AddThread(&DisplayThread, 128, 6);
  while(NumSamples < RUNLENGTH) {
//    sprintf(str, "Time left is %d", (RUNLENGTH-NumSamples)/1000);
...
//*******attach background tasks***********
  ...
  DAS_id = OS_Add_Periodic_Thread(&DAS,PERIOD,1); // 2 kHz real time sampling
...
// no UART1 interrupts
// SYSTICK interrupts, with or without period established by OS_Launch
// Timer2 interrupts, with or without period established by OS_Add_Periodic_Thread
...
void Thread2c(void){
  ...
  OS_Add_Periodic_Thread(&BackgroundThread1c,TIME_1MS,0); 
  for(;;){
    OS_Wait(&Readyc);
...
// no UART interrupts
// SYSTICK interrupts, with or without period established by OS_Launch
// Timer interrupts, with or without period established by OS_Add_Periodic_Thread
...
int Testmain4(void){   // Testmain4
  ...
  OS_Init();           // initialize, disable interrupts
  NumCreated = 0 ;
  OS_Add_Periodic_Thread(&BackgroundThread1d,PERIOD,0); 
...
// first timer interrupts, period established by first call to OS_Add_Periodic_Thread
// second timer interrupts, period established by second call to OS_Add_Periodic_Thread
// Select no interrupts
// Down no interrupts
unsigned long CountA;   // number of times Task A called
unsigned long CountB;   // number of times Task B called
unsigned long Count1;   // number of times thread1 loops
// ids for periodic tasks, used to record jitter
int TaskAID;
int TaskBID;
// period for periodic tasks in ms
unsigned long TaskAPeriod = 1;
unsigned long TaskBPeriod = 2;
...
// Input: amount of work in 1us units (free free to change units
// Output: none
...
//extern void SH_Jitter(void);   // prints jitter information (write this)
void Thread7(void){  // foreground thread
//   printf("\n\rEE345M/EE380L, Lab 3 Preparation 2\n\r");
  OS_Sleep(5000);   // 10 seconds        
  SH_Jitter();         // print jitter information
//   printf("\n\r\n\r");
  OS_Kill();
}
...
void TaskB(void){       // called every pB in background
  ...
  OS_MeasureJitter(TaskBID, TaskBPeriod * 1000);
...
int main5(void){       // Testmain5
  ...
  TaskAID = OS_Add_Periodic_Thread(&TaskA,TaskAPeriod,0);           // 1 ms, higher priority
  TaskBID = OS_Add_Periodic_Thread(&TaskB,TaskBPeriod,1);         // 2 ms, lower priority
...
// select interrupts, 
// down no interrupts
OS_SemaphoreType s;            // test of this counting semaphore
...
int main6(void){      // Testmain6
  ...
  OS_Add_Periodic_Thread(&Signal1,1,0);   // 0.799 ms, higher priority
  OS_Add_Periodic_Thread(&Signal2,2,1);  // 1.111 ms, lower priority
  NumCreated = 0 ;
