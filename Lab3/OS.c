...
typedef struct _TCB {
  volatile unsigned long * sp;    /* stack pointer */
	...
	unsigned long base_priority;		/* Base thread priority */
...
typedef struct _OS_FifoType {
  ...
  OS_SemaphoreType notEmpty; // TODO
  OS_SemaphoreType mutex;    // TODO
} _OS_FifoType;

typedef struct _OS_MailBoxType {
  unsigned long data;
  OS_SemaphoreType hasData;  // TODO
  OS_SemaphoreType gotData;  // TODO
} _OS_MailboxType;
...
void Timer2B_Init(int priority) {
  int nop = 5;
	...
	TIMER2_TBPR_R = 0;											/* 1us timer2B */
	TIMER2_ICR_R |= 0x00000100;							/* Clear timer2B timeout flag */
	TIMER2_TBILR_R |= 0x0000FFFF;									/* Reload time of 100 us */
...
void Timer2A_Handler(void)
{
  ...
  if((_us100Count % 10) == 0) {
    ...
    for(i = 0; i < _OS_MAX_THREADS; i++) {
      if(_threads[i].id != _OS_FREE_THREAD && _threads[i].sleep) { // kinda hacky
...
int OS_AddThread(void(*task)(void), unsigned long stackSize, unsigned long priority) {
  ...
  if(_RunPt == NULL) {
    ...
    if(i == _OS_MAX_THREADS) {
      ...
      // TODO - handle this more elgeantly
...
void OS_Kill(void)
{
  ...
  OS_ENTER_CRITICAL();
	...
	temp = _RunPt;
	_RunPt = _RunPt->next;  // TODO - could a systick interrupt cause the next thread to be skipped?
...
static void (*PortETask)(void) = NULL;

void OS_AperiodicTask(void(*task)(void), unsigned long priority)
{
	PortETask = task;
...
void OS_RemoveAperiodicTask(void)
{
	PortETask = NULL;
}
...
void PORTE_Init(void) {
  ...
  GPIO_PORTE_IBE_R &= ~PORTE_PINS;  // interrupt both edges
