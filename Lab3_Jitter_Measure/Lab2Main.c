...
int DASPeriodicID;          // id of the periodic task for the DAS
...
//   MinJitter = 10000000;
  ...
  DASPeriodicID = OS_Add_Periodic_Thread(&DAS,2,1); // .5 kHz real time sampling (breaks project currently)
...
//     thisTime = OS_Time();       // current time, 20 ns
    ...
    OS_MeasureJitter(DASPeriodicID, 2000);  // assuming this is periodic task id 1, period 2000 usec
...
unsigned long myId = OS_Id(); 
  ...
  OLED_Out(BOTTOM, str);
  sprintf(str, "Jitter(us) = %d",OS_getMaxJitter(DASPeriodicID)-OS_getMinJitter(DASPeriodicID));
  OLED_Out(BOTTOM, str);
  OLED_Out(BOTTOM, "");
  OS_Kill();  // done
