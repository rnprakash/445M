...
static char _SH_History[SH_HISTORY][SH_MAX_LENGTH];
static int index = 0;
  ...
  while(character != CR && character != LF && character != CTRL_C){
    ...
    if(character == BS || character == DEL){
      if(length){
		...
		{
			char fBuff[8];
			if(space) // autocomplete file name
			{
				char *c = fBuff;
			...
			else // autocomplete command name
			{
				char *c = fBuff;
						...
						&& startPt[length-2] == 0x1B) // down arrow
		{
			...
			if(!_SH_History[(index+1)%SH_HISTORY][0])
				continue;
...
static unsigned int(*time)(void) = &OS_MsTime;
...
static int _SH_Create(void)
{
	...
	for(i = 0; _SH_cmd.args[i][0] && i < SH_MAX_ARGS; i++)
		if(eFile_Create(_SH_cmd.args[i]))
