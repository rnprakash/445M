...
OS_SemaphoreType display_semaphore;
...
int main(void)
{
  ...
  OS_Fifo_Init(MAX_FIFO_SIZE);
	OS_InitSemaphore(&display_semaphore, 1);
...
//  OS_AddButtonTask(&dummyButtonTask, 1);
  ...
  OS_AddDownTask(&dummyDownTask, 2);
//   NumCreated += OS_AddThread(&dummyTask3, 0, 1);
//   NumCreated += OS_AddThread(&dummyTask1, 0, 2);
//   NumCreated += OS_AddThread(&PID, 0, 3);
//   NumCreated += OS_AddThread(&dummyTask2, 0, 3);
//   NumCreated += OS_AddThread(&Consumer, 128, 1);
  NumCreated += OS_AddThread(&oledTestTask, 128, 1);
...
void oledTestTask(void) {
  while(1) {
    ...
    RIT128x96x4StringDraw("This is a test string", 0, 8, 0xF);
...
//       OS_LogEvent(EVENT_CONSUMER_GOT);
...
{
	char str[20];
	OS_bWait(&display_semaphore);
	if((RUNLENGTH-NumSamples)/1000 > 0)
	{
