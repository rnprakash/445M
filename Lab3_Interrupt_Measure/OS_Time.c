...
static unsigned long _OS_tot_time = 0;
...
void OS_end_interrupt(void)
{
	...
	temp = OS_TimeDifference(_OS_cur_time, temp);
	if(temp > _OS_int_time)
		_OS_int_time = temp;
	_OS_tot_time += temp;
}

// in 20ns units (i.e. cycles)
...
{
	return _OS_tot_time * 100 / 50000 / OS_MsTime();
}
