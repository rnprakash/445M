...
//      UART_OutString(_SH_getVar(command.arguments[0]));
    ...
    case 'v':
      // print FFT log
      exitCode = SH_PrintFFTLog();
      break;
    case 'b':
      // clear FFT log
      exitCode = SH_ClearFFTLog();
      break;
    default:
...
extern long FFT_Log[64];
extern long FFT_LogIndex;
int SH_PrintFFTLog(void) {
  int i;
  // output as tab-seperated values
  for(i = 0; i < FFT_LogIndex; i++) {
    UART_OutUDec(FFT_Log[i]);UART_OutString("\t");
  }
  return 0;
}
...
int SH_ClearFFTLog(void) {
  FFT_LogIndex = 0;
  UART_OutString("Cleared FFT Log");
  return 0;
}
